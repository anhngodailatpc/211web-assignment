      ...
      for (var key in response[0]) {
        ...
        if (key != 'name') {
          ...
          for (var i = 0; i < len; i++) {
            ...
            if (t === 1)
              tr_str =
                tr_str + '<td><span class="my-ok-icon">&#10003;</span></td>';
            else if (t === 0) tr_str = tr_str + '<td>-</td>';
            else {
              if (key == 'monthlyPrice')
                tr_str =
                  tr_str +
                  '<td><span class="strong-text">' +
                  t +
                  '</span><span class="sub-align">/1thang</span></td>';
              else tr_str = tr_str + '<td>' + t + '</td>';
            }
          }
          tr_str = tr_str + '</tr>';
          row = row + 1;
