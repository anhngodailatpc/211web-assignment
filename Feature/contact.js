$(document).ready(function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (userrole == 'guess')
        postsHTML +=
          "<div class='textarea_cmt col-sm'><textarea disabled placeholder='Vui lòng đăng nhập để bình luận'></textarea></div> </div>";
      else
        postsHTML +=
      ...
      for (var id in posts) {
        ...
        if (posts[id].userid === result['userID']) {
          row +=
            `<button class='edit_post' id='edit_btn_${id}'>Sửa</button>` +
            `<button class='delete_post' id='delete_btn_${id}'>Xóa</button>`;
        } else if (+result['role'] === 3) {
          row += `<button class='delete_post' id='delete_btn_${id}'>Xóa</button>`;
        }
